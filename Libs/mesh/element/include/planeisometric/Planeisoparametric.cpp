    ...
    const std::shared_ptr<Material>& material) const
{
  ...
  /*  std::cout << "=== B-matrix Debug ===" << std::endl;
    ...
    // ѕреобразование производных в глобальные координаты
    for (int i = 0; i < 4; ++i) {
...
Eigen::MatrixXd PlaneIsoparametricElement::getNodalCoordinates(const std::vector<std::shared_ptr<Node>>& nodes) const {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // Ќаходим узел по ID (здесь нужна оптимизаци€ - кэш или быстрый поиск)
