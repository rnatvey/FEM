...
public:
    ...
    Eigen::MatrixXd computeStiffnessMatrix(const std::vector<std::shared_ptr<Node>>& nodes, 
    ...
    Eigen::Matrix2d jacobian(double xi, double eta, const std::vector<std::shared_ptr<Node>>& nodes) const override;
    ...
    bool isValid(const std::vector<std::shared_ptr<Node>>& nodes) const override;
