...
public:
    ...
    Eigen::MatrixXd computePenaltyStiffnessMatrix(const std::vector<Node>& nodes,
...
private:
    ...
    double calculateGap(int surfaceIndex, const Eigen::VectorXd& displacements,
        const std::vector<Node>& nodes) const;
    Eigen::MatrixXd computeSurfaceStiffness(int surfaceIndex, const std::vector<Node>& nodes,
        const Material& material) const;
};