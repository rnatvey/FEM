...
enum class ContactMethodVariant {
    Penalty,
    AugmentedLagrangian,
};
...
struct StudyResult {
    ContactMethodVariant method = ContactMethodVariant::Penalty;
    std::string contactMethodName;
    std::string contactParameterName;
    std::string meshLabel;
    double contactParameterValue = 0.0;
    bool success = false;
...
bool quickModeEnabled() {
    if (const char* quickModeValue = std::getenv("FEM_MAIN_SCALE_COMPARISON_QUICK");
...
std::vector<ContactMethodVariant> buildContactMethods() {
    return {
        ContactMethodVariant::Penalty,
        ContactMethodVariant::AugmentedLagrangian,
    };
}

std::string contactMethodName(const ContactMethodVariant method) {
    switch (method) {
...
std::string contactParameterName(const ContactMethodVariant method) {
    switch (method) {
    case ContactMethodVariant::Penalty:
...
std::string makeCaseToken(const ContactMethodVariant method,
    const std::string& meshLabel,
    double contactParameterValue) {
    std::ostringstream stream;
    stream << contactMethodName(method) << "_"
           << meshLabel << "_"
           << contactParameterName(method) << "_"
           << std::llround(contactParameterValue);
    return stream.str();
}
...
std::string buildStudyRow(const StudyResult& result) {
    ...
    stream << std::fixed << std::setprecision(8)
           << result.contactMethodName << ','
           << result.contactParameterName << ','
           << result.contactParameterValue << ','
           << result.meshLabel << ','
...
MeshGenerator::TireContactAnalysisSetup buildTireContactSetup(
    const std::shared_ptr<Assembly>& assembly,
    ...
    return meshGenerator.generateTireContactAnalysisSetup(control);
}

StudyResult runStudyCase(const std::filesystem::path& outputRoot,
    const ContactMethodVariant method,
    const MeshVariant& meshVariant,
    double contactParameterValue) {
    StudyResult result;
    result.method = method;
    result.contactMethodName = contactMethodName(method);
    result.contactParameterName = contactParameterName(method);
    result.meshLabel = meshVariant.label;
    result.contactParameterValue = contactParameterValue;
    ...
    const auto setup = buildTireContactSetup(assembly, meshVariant);

    FEModel model;
    model.setAssembly(assembly);
    model.setSolverTolerance(1.0e-9);

    if (method == ContactMethodVariant::Penalty) {
    ...
    exportOptions.outputDirectory = outputRoot /
        makeCaseToken(method, meshVariant.label, contactParameterValue);
    exportOptions.baseName = "solution";
    exportOptions.extraStringMetrics = {
        {"case_name", makeCaseToken(method, meshVariant.label, contactParameterValue)},
        {"mesh_label", meshVariant.label},
        {"geometry_family", "tire_ring"},
        {"study_family", "main_scale_contact_method_comparison"},
        {"contact_method", result.contactMethodName},
        {"contact_parameter_name", result.contactParameterName}
    };
    ...
    try {
        ...
        for (const auto method : methods) {
            ...
            for (const auto& meshVariant : meshVariants) {
                ...
                for (double contactParameterValue : contactParameterValues) {
                    if (!matchesOptionalNumericFilter(parameterFilter, contactParameterValue)) {
                        continue;
                    }
                    const StudyResult result = runStudyCase(
                        outputRoot,
                        method,
                        meshVariant,
                        contactParameterValue);
        ...
        std::cerr << "main_scale_contact_method_comparison_study_failed: "
