...
int main() {
    try {
        ...
        const AugmentedLagrangianSettings settings{1.0e7, 1.0e-8};
        contactModel->configureRigidPlaneAugmentedLagrangianContact(farPlane, contactFacets, settings);
        require(contactModel->solveContact(),
            "solveContact() failed in augmented Lagrangian no-contact scenario");
        ...
        require(metrics.activeContactGaussPoints == 0,
            "No-contact scenario must keep all contact Gauss points inactive");
        ...
        require(metrics.maxNormalContactMultiplier == 0.0,
            "No-contact scenario must keep all normal multipliers equal to zero");

        std::cout << "contact_no_contact_augmented_lagrangian_regression_passed" << std::endl;
        ...
        std::cerr << "contact_no_contact_augmented_lagrangian_regression_failed: "
