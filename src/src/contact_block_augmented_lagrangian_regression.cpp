...
constexpr double kAugmentationParameter = 1.0e7;
...
int main() {
    try {
        ...
        ContactState precontactState = model->evaluateCurrentContactState();
        RigidPlaneContactSolver precheckSolver(assembly, rigidPlane, kAugmentationParameter);
        ...
        model->configureRigidPlaneAugmentedLagrangianContact(
            rigidPlane, contactFacets, AugmentedLagrangianSettings{kAugmentationParameter, 1.0e-8});
        require(model->solveContact(), "Augmented Lagrangian contact solve failed");
        ...
        require(metrics.contactMethod == "augmented_lagrangian",
            "Contact method label is not augmented_lagrangian");
        require(metrics.nonlinearIterations > 0, "Nonlinear iteration counter was not updated");
        require(metrics.nonlinearIterations <= 30,
            "Augmented Lagrangian contact solve required too many nonlinear iterations");
        require(metrics.activeSetSize == kExpectedActiveFacets,
            "Unexpected converged active set size");
        require(metrics.activeContactGaussPoints > 0,
            "Augmented Lagrangian solve must activate contact Gauss points");
        require(metrics.maxNormalContactMultiplier > 0.0,
            "Augmented Lagrangian solve must build positive normal multipliers");
        require(metrics.contactStateRelativeUpdateNorm <= 1.0e-8,
            "Normal multiplier update did not converge to the requested tolerance");
        ...
        std::cout << "contact_block_augmented_lagrangian_regression_passed" << std::endl;
        ...
        std::cout << "lambda_relative_update=" << metrics.contactStateRelativeUpdateNorm << std::endl;
        ...
        std::cerr << "contact_block_augmented_lagrangian_regression_failed: "
